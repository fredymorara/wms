...
import { applyForCampaign } from '../services/api';
...
const MemberCampaignApplicationModal = ({ visible, onCancel, onCreated }) => {
    ...
    const onFinish = async (values) => {
        ...
        try {
            ...
            await applyForCampaign(payload);
            message.success({ content: 'Campaign application submitted successfully. Awaiting Admin approval.', className: 'rounded-xl font-medium' });
            onCancel();
            if (onCreated) {
                onCreated();
            }
            ...
            setError(e.message);
            message.error({ content: `Campaign application failed: ${e.message}`, className: 'rounded-xl font-medium' });
        } finally {
            setLoading(false);
        }
    };
        ...
        >
            <div className="pt-6">
                ...
                >
                    ...
                    >
                        <Input size="large" className="rounded-xl border-zinc-300 h-12" placeholder="e.g., Medical Assistance for Surgery" />
                    </Form.Item>

                    <Form.Item
                        label={<span className="font-semibold text-zinc-700">Brief Description</span>}
                        name="description"
                        rules={[{ required: true, message: 'Please enter campaign description!' }]}
                    >
                        <TextArea rows={3} className="rounded-xl border-zinc-300 p-3" placeholder="A short summary of your situation..." />
                    </Form.Item>

                    <Form.Item
                        label={<span className="font-semibold text-zinc-700">Detailed Information</span>}
                        name="details"
                    >
                        <TextArea rows={5} className="rounded-xl border-zinc-300 p-3" placeholder="Provide full details, background context, and breakdown of needs..." />
                    </Form.Item>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                        ...
                        <Form.Item
                            label={<span className="font-semibold text-zinc-700">Required Amount (KES)</span>}
                            name="goalAmount"
                            rules={[{ required: true, message: 'Please enter the required amount!' }]}
                        >
                            <Input type="number" size="large" className="rounded-xl border-zinc-300 h-12" placeholder="e.g., 20000" />
                        </Form.Item>
                    </div>

                    <Form.Item
                        label={<span className="font-semibold text-zinc-700">Desired Deadline</span>}
                        name="endDate"
                ...
                {loading && <div className="flex justify-center mt-6"><Spin tip="Submitting Application..." /></div>}
            </div>
        </Modal>
    );
};
